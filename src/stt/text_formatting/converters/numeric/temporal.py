"""Temporal converter for time-related numeric entities."""
...
class TemporalConverter(BaseNumericConverter):
    """Converter for temporal patterns like time, duration, and relative time expressions."""
    
    def __init__(self, number_parser, language: str = "en"):
        """Initialize temporal converter."""
    ...
    def convert(self, entity: Entity, full_text: str = "") -> str:
        """Convert a temporal entity to its final form."""
    ...
    def convert_time_duration(self, entity: Entity) -> str:
        ...
        if entity.metadata.get("is_compound"):
            ...
            abbrev1 = self.time_duration_unit_map.get(unit1, unit1)
            abbrev2 = self.time_duration_unit_map.get(unit2, unit2)
        ...
        if "number" in entity.metadata and "unit" in entity.metadata:
            ...
            abbrev = self.time_duration_unit_map.get(unit, unit)
    ...
    def convert_time_or_duration(self, entity: Entity) -> str:
        ...
        if match:
            ...
            # Get abbreviated units
            abbrev1 = self.time_duration_unit_map.get(unit1.lower(), unit1)
            abbrev2 = self.time_duration_unit_map.get(unit2.lower(), unit2)
        ...
        if match:
            ...
            # Get abbreviated unit
            abbrev = self.time_duration_unit_map.get(unit.lower(), unit)
    ...
    def convert_time_relative(self, entity: Entity) -> str:
        ...
        hour = self.hour_mappings.get(hour_word)
        if hour is None:
