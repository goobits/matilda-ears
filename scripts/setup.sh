...
# ║                        AUTO-GENERATED FILE                               ║
# ║                                                                           ║
# ║  Generated by: goobits-cli v1.1.1                     ║
# ║  Generated from:                                          ║
# ║                                                                           ║
...
readonly PACKAGE_NAME="goobits-matilda-ears"
readonly COMMAND_NAME="ears"
readonly DISPLAY_NAME="Matilda Ears - Speech-to-Text Engine"
readonly DESCRIPTION="Pure speech-to-text engine with multiple operation modes"
readonly PYPI_NAME="goobits-matilda-ears"
readonly DEVELOPMENT_PATH="."
readonly REQUIRED_VERSION="3.11"
...
install_with_pipx() {
    ...
    if [[ "$install_dev" == "true" ]]; then
        ...
        (cd "$PROJECT_DIR" && pipx install --editable "$DEVELOPMENT_PATH[dev,whisper,vad]" --force) &
        local install_pid=$!
        ...
        pipx install "$PYPI_NAME[dev,whisper,vad]" --force &
        local install_pid=$!
...
install_with_pip() {
    ...
    if [[ "$install_dev" == "true" ]]; then
        ...
        (cd "$PROJECT_DIR" && python3 -m pip install --editable "$DEVELOPMENT_PATH[dev,whisper,vad]" $PIP_USER_FLAG) &
        show_spinner $!
        wait $!
        local exit_code=$?
        ...
        python3 -m pip install "$PYPI_NAME[dev,whisper,vad]" $PIP_USER_FLAG &
        show_spinner $!
        wait $!
        local exit_code=$?
...
upgrade_package() {
    ...
    if [[ "$PIPX_AVAILABLE" == "true" ]]; then
        ...
        python3 -m pip install --upgrade "$PYPI_NAME[dev,whisper,vad]" $PIP_USER_FLAG >/dev/null 2>&1 &
        show_spinner $!
        wait $!
        local exit_code=$?
    fi
...
show_dev_success_message() {
    echo
    echo "Matilda Ears has been installed in development mode!
...
show_upgrade_success_message() {
    echo
    echo "Matilda Ears has been upgraded successfully!
...
show_uninstall_success_message() {
    echo
    echo "Matilda Ears has been uninstalled.
